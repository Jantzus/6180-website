...
// Styled components for tag filter display (removed FilterContainer)
...
// TagsFilter Component with hierarchical multi-select filtering
...
export const TagsFilter: React.FC<TagsFilterProps> = ({
  ...
  const filterFolders = (selectedKeys: string[]) => {
    ...
    // Filter folders to only those containing ALL selected tags/subtags
    const newFilteredFolders = folders.filter(folder => {
      // Get all tag keys from all files in this folder
      ...
      // Check if ALL selected tags exist in this folder's files
      return selectedKeys.every(selectedKey => 
        folderTagKeys.has(selectedKey)
      );
    });
    
    // Update visible tags based on the filtered folders
  ...
  const updateVisibleTags = (filteredFolders: FolderType[]) => {
    // Extract all unique tag keys from the filtered folders
