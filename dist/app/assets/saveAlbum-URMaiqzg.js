import{d as z,u as Ke,a as y,j as t,g as Ie,r as Ge,h as de,L as me,i as Fe,s as Tt,m as vt,f as ye,k as It,l as Pt,n as At,R as kt,I as jt,p as Ct}from"./utils-C1FUk2iD.js";import{S as Dt,C as Pe,F as _e,e as Ee,f as Ft,g as _t,T as Ne,h as Re,i as Oe,j as ze,P as Et,k as Nt,l as qe,m as He,n as Rt,o as Ot,p as zt,q as Bt,V as Mt,r as Ut,s as Wt,M as Kt,G as Gt,A as qt,H as Ht,t as Jt,u as Qt,B as ve}from"./styled-components-BlKL9vJ2.js";import{u as Vt,U as Yt}from"./useUsernameManagement-CK8PXyEj.js";import{u as Xt,U as Zt}from"./useFileUploadProcessor-DFxO8lXR.js";import{F as Lt}from"./types-B2_92tNb.js";import{L as es}from"./LazyImage-CPKlzoGh.js";const d={colors:{primary:"#007bff",primaryDark:"#0056b3",secondary:"#6c757d",success:"#4caf50",danger:"#e53935",warning:"#ff9800",white:"#fff",text:{primary:"#333",secondary:"#666",light:"#777"},background:{primary:"#f9fafb",highlight:"#f0f7ff"},border:"#ddd"},spacing:{xs:"4px",sm:"8px",md:"16px",lg:"24px",xl:"32px"},borderRadius:{small:"4px",medium:"8px"},boxShadow:{lg:"0 4px 10px rgba(0,0,0,0.08)"}},ts=z.div`
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
...
Data: ${G}`,console.log(R),console.log("Data object:",w)}catch(G){R+=` [Error stringifying data: ${G}]`,console.log(R),console.log("Raw data:",w)}else console.log(R);Se(R)},tt=async a=>{var w,N,R,G;if(a){ue(!0),k(`Fetching existing album data for folder ID: ${a}`);try{const re=await de();if(!re){k("❌ Authentication failed while fetching existing album data");return}const St=`
      ...
      `,$t={fetchRelationsInput:{ownerItemId:"myAccountOwnerItemId",rangeKeyPrefix:"FolderPosition",index:"ownerItemId_____RelationType____sortParameter",limit:2e3,scanIndexForward:!1}},we=await(await fetch(ye,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${re}`},body:JSON.stringify({query:St,variables:$t})})).json();if(we.errors){console.error("GraphQL errors:",we.errors),k(`❌ Failed to fetch existing album data: ${JSON.stringify(we.errors)}`);return}const Ce=(((N=(w=we==null?void 0:we.data)==null?void 0:w.fetchRelations)==null?void 0:N.items)||[]).find(ne=>ne&&ne.folder&&ne.folder.id===a);if(!Ce)return;const he=Ce.folder,De=(((G=(R=he==null?void 0:he.fileReferencesPage)==null?void 0:R.items)==null?void 0:G.map(ne=>ne.file))||[]).filter(ne=>ne&&ne.dataKey).map(ne=>({dataKey:ne.dataKey,thumbnailDataKey:ne.thumbnailDataKey||null,durationInSeconds:ne.durationInSeconds||null,dataInBytes:ne.dataInBytes||0}));M(De),k(`✅ Successfully loaded ${De.length} existing files`),!o&&he.folderName&&H(he.folderName),!q&&he.folderDescription&&j(he.folderDescription),ge(a)}catch(re){console.error("Failed to fetch existing album data:",re),k(`❌ Failed to fetch existing album data: ${String(re)}`)}finally{ue(!1)}}};y.useEffect(()=>{c&&c!==Y&&(k(`Loading existing files for folder ID: ${c}`),tt(c))},[c,Y]);const st=_s(k),{cognitoUsername:ke,publicUsername:$e,setPublicUsername:rt}=Ts(v,A,ee,Z,H,j,F,U,ae,l,x,_,k),{saveAlbumDirectly:je}=vs(c||Y,ke,f,fe,b,o,q,B,m,V,D,T,$,u,A,se,k);y.useEffect(()=>{Y&&!c&&(v(Y),k(`Updated folder ID from upload processor: ${Y}`))},[Y,c]),y.useEffect(()=>{C(a=>{const w=new Set;return a.forEach(N=>{N<f.length&&w.add(N)}),w}),E(a=>{const w=new Map(a),N=[];return a.forEach((R,G)=>{G>=f.length&&N.push(G)}),N.forEach(R=>{w.delete(R)}),w})},[f.length]),y.useEffect(()=>{ie(a=>{const w=new Set;return a.forEach(N=>{N<P.length&&w.add(N)}),w}),pe(a=>{const w=new Map(a),N=[];return a.forEach((R,G)=>{G>=P.length&&N.push(G)}),N.forEach(R=>{w.delete(R)}),w})},[P.length]);const it=a=>{k(`Removing photo at index: ${a}`);const w=f.filter((N,R)=>R!==a);A(w),k(`New files count: ${w.length}`),w.length>0?(localStorage.setItem(me.SELECTED_PHOTOS,JSON.stringify(w)),k(`Updated localStorage with ${w.length} photos`)):(localStorage.removeItem(me.SELECTED_PHOTOS),k("Removed photos from localStorage")),C(N=>{const R=new Set;return N.forEach(G=>{G<a?R.add(G):G>a&&R.add(G-1)}),R}),E(N=>{const R=new Map;return N.forEach((G,re)=>{re<a?R.set(re,G):re>a&&R.set(re-1,G)}),R})},ot=a=>{k(`Removing existing file at index: ${a}`);const w=P.filter((N,R)=>R!==a);M(w),k(`New existing files count: ${w.length}`),ie(N=>{const R=new Set;return N.forEach(G=>{G<a?R.add(G):G>a&&R.add(G-1)}),R}),pe(N=>{const R=new Map;return N.forEach((G,re)=>{re<a?R.set(re,G):re>a&&R.set(re-1,G)}),R})},at=a=>{k(`Toggling selection for existing file at index: ${a}`),ie(w=>{const N=new Set(w);return N.has(a)?(N.delete(a),k(`Deselected existing file ${a}`)):(N.add(a),k(`Selected existing file ${a}`)),N})},nt=()=>{k("Selecting all existing files");const a=new Set;for(let w=0;w<P.length;w++)a.add(w);ie(a)},lt=()=>{k("Deselecting all existing files"),ie(new Set)},dt=a=>{k(`Toggling selection for photo at index: ${a}`),C(w=>{const N=new Set(w);return N.has(a)?(N.delete(a),k(`Deselected photo ${a}`)):(N.add(a),k(`Selected photo ${a}`)),N})},ct=()=>{k("Selecting all photos");const a=new Set;for(let w=0;w<f.length;w++)a.add(w);C(a)},ut=()=>{k("Deselecting all photos"),C(new Set)},pt=()=>{const a=!B;k(`Toggling isOnPublicProfile to: ${a}`),F(a)},ft=()=>{const a=!m;k(`Toggling participantsCanAddItems to: ${a}`),U(a)},gt=async()=>{k("Album save initiated"),k("Photo tags applied:",Object.fromEntries(T)),k("Existing file tags applied:",Object.fromEntries(oe)),$(!0);try{if($e!=null&&$e.startsWith("Profile-")){k("Public username starts with 'Profile-', showing username prompt"),I(""),p(!0),$(!1);return}k("Valid username found, proceeding to save album directly with file-level tagging"),je()}catch(a){console.error("Error in handleSaveAlbum:",a),k(`Error in handleSaveAlbum: ${a}`),$(!1)}},mt=a=>{k(`Handling successful username update to: ${a}`),localStorage.setItem(me.PUBLIC_USERNAME,a),rt(a),p(!1),k("Proceeding to save album after username update"),je()},xt=(a,w)=>{k(`Password dialog closed with option: ${a}, password: ${w?"******":"undefined"}`),a&&ae(a),w!==void 0&&l(w),L(!1)},ht=()=>V==="NoPassword"?e("Album Password Policy"):`${e(V==="NotVisible"?"Password Required To See Or Save":V==="Watermark"?"Watermarked And No Saving Without Password":"Password Required To Save")} ${D?`(${D})`:""}`,bt=()=>{k("Opening password dialog"),L(!0)},wt=()=>{k("Add photos button clicked"),le(c)},Te=g||Q||ce,yt=f.length>0||P.length>0;return t.jsxs(t.Fragment,{children:[t.jsx(Gt,{}),t.jsxs(qt,{$isRTL:i,children:[t.jsxs("div",{style:{maxWidth:"900px",margin:"0 auto"},children:[t.jsxs(Ht,{children:[t.jsx(Jt,{href:"my-albums.html",children:e("My Albums")}),t.jsx(ys,{t:e})]}),t.jsx(As,{showFolderDetails:s,isCreator:O,folderName:o,setFolderName:H,folderDescription:q,setFolderDescription:j,isOnPublicProfile:B,handlePublicProfileToggle:pt,participantsCanAddItems:m,handleParticipantsCanAddItemsToggle:ft,isSavingAlbum:g||Q}),(Q||K.totalFiles>0&&(K.filesUploading>0||K.filesProcessing>0||K.filesComplete<K.totalFiles))&&t.jsx(Zt,{progressTracker:K,isRTL:Ie(n)==="rtl",variant:"detailed",context:"saving",isUploading:Q,showSuccessMessage:!1,showErrorMessage:!0,customMessages:{error:e("Some photos could not be processed. You can continue with the successfully processed photos.")}}),t.jsx("input",{ref:r,id:"file-input",type:"file",accept:"image/*,video/*",multiple:!0,onChange:a=>xe(a,ke),style:{display:"none"}}),s&&O===!0&&yt&&t.jsxs(Gs,{t:e,isRTL:i,children:[(P.length>0||ce)&&t.jsx("div",{style:{marginBottom:"24px"},children:ce?t.jsx("div",{style:{padding:"20px",textAlign:"center",color:"#666",fontSize:"14px"},children:e("Loading existing files...")}):t.jsx(Hs,{existingFiles:P,selectedExistingIndices:J,onToggleSelection:at,onSelectAll:nt,onDeselectAll:lt,onDeleteFile:ot,disabled:Te,t:e,isRTL:i})}),t.jsx(qs,{selectedPhotos:f,selectedPhotoIndices:h,onToggleSelection:dt,onSelectAll:ct,onDeselectAll:ut,onRemovePhoto:it,disabled:Te,t:e,isRTL:i}),t.jsx("div",{style:{marginTop:"24px"},children:t.jsx(Ws,{tagsManager:st,disabled:Te,enhancedLog:k,photoCount:f.length+P.length})})]}),t.jsx(Ps,{isSavingAlbum:g,savingProgress:W}),t.jsxs(Qt,{children:[t.jsx(ve,{onClick:wt,disabled:g||Q||ce,children:e(Q?"Uploading...":"Add More Photos")}),O===!0&&t.jsx(ve,{$passwordSet:V!=="NoPassword",onClick:bt,disabled:g||Q||ce,children:ht()}),t.jsx(ve,{$primary:!0,onClick:gt,disabled:g||Q||ce,children:e(g?"Saving Album...":"Save Album")})]}),t.jsx(Yt,{t:e,language:n,usernameManager:S,onSuccess:mt}),t.jsx(ws,{isOpen:X,onClose:xt,initialOption:V,initialPassword:D})]}),t.jsx(Ss,{debugMessages:te,t:e,isRTL:i,textDirection:i?"rtl":"ltr"})]})]})},Qs=()=>t.jsx(jt,{children:t.jsx(Js,{})});kt.createRoot(document.getElementById("root")).render(t.jsx(Qs,{}));
