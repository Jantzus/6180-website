...
Data: ${Q}`,console.log(C),console.log("Data object:",A)}catch(Q){C+=` [Error stringifying data: ${Q}]`,console.log(C),console.log("Raw data:",A)}else console.log(C);_e(C)},gt=async a=>{var A,D,C;if(a){S(!0),v(`Fetching existing album data for folder ID: ${a}`);try{const Q=await xe();if(!Q){v("❌ Authentication failed while fetching existing album data");return}const fe=`
      ...
      `,Mt={fetchRelationsInput:{ownerItemId:"myAccountOwnerItemId",rangeKeyPrefix:"FolderPosition",index:"ownerItemId_____RelationType____sortParameter",limit:2e3,scanIndexForward:!1}},Ae=await(await fetch(je,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${Q}`},body:JSON.stringify({query:fe,variables:Mt})})).json();if(Ae.errors){console.error("GraphQL errors:",Ae.errors),v(`❌ Failed to fetch existing album data: ${JSON.stringify(Ae.errors)}`);return}const He=(((D=(A=Ae==null?void 0:Ae.data)==null?void 0:A.fetchRelations)==null?void 0:D.items)||[]).find(Se=>Se&&Se.folder&&Se.folder.id===a);if(!He)return;const ve=He.folder,Bt=((C=ve==null?void 0:ve.fileReferencesPage)==null?void 0:C.items)||[],Ce=[],Re=new Map;Bt.forEach((Se,Ke)=>{const Pe=Se.file;if(Pe&&Pe.dataKey){const qe=Se.fileDisplayName||null;Ce.push({dataKey:Pe.dataKey,thumbnailDataKey:Pe.thumbnailDataKey||null,durationInSeconds:Pe.durationInSeconds||null,dataInBytes:Pe.dataInBytes||0,fileName:qe});const Ge=Se.selectedTags||[];if(Ge.length>0){const Oe=Ge.map(ke=>{var Je;return{tagTitle:ke.tagTitle,TagType:ke.TagType,subtags:((Je=ke.subtags)==null?void 0:Je.map(Qe=>({tagTitle:Qe.tagTitle,subtagTitle:Qe.subtagTitle})))||[]}});Re.set(Ke,Oe),v(`Existing file ${Ke} (${qe||"unnamed"}) has ${Oe.length} tags applied:`,Oe.map(ke=>ke.tagTitle))}}}),V(Ce),ge(Re),v(`✅ Successfully loaded ${Ce.length} existing files with ${Re.size} files having existing tags and ${Ce.filter(Se=>Se.fileName).length} files with names`),!u&&ve.folderName&&o(ve.folderName),!q&&ve.folderDescription&&R(ve.folderDescription)}catch(Q){console.error("Failed to fetch existing album data:",Q),v(`❌ Failed to fetch existing album data: ${String(Q)}`)}finally{S(!1)}}};f.useEffect(()=>{const A=new URLSearchParams(window.location.search).get("folderId");if(A){v(`Found folderId query parameter: ${A} - loading existing album`),I(!0),h(A),ye(!1),localStorage.removeItem(me.SUB_ALBUM_DATA),v("Cleared sub-album data from localStorage - loading existing album from query parameter");const D=localStorage.getItem(me.SELECTED_PHOTOS);if(!D)v("No stored photos found - proceeding with existing album load");else try{const C=JSON.parse(D);Array.isArray(C)&&C.length>0?v(`Found ${C.length} stored photos - these may be newly uploaded for this album, preserving them`):(localStorage.removeItem(me.SELECTED_PHOTOS),v("Cleared empty photos array from localStorage"))}catch(C){v(`Error parsing stored photos: ${C}`),localStorage.removeItem(me.SELECTED_PHOTOS)}ne(!0),ue(!0)}else v("No folderId query parameter found, will proceed with normal initialization"),I(!1)},[]);const{cognitoUsername:Ue,publicUsername:Ne,setPublicUsername:mt}=Us(de?()=>{}:h,x,de?()=>{}:ue,de?()=>{}:ne,o,R,ee,Y,ae,M,de?()=>{}:ye,de?()=>{}:Ie,O,v);f.useEffect(()=>{i&&(v(`Loading existing files for folder ID: ${i}`),gt(i))},[i]);const xt=or(E,G,ie,ge,T,B,v),{saveAlbumDirectly:We}=Ws(i||X,Ue,P,Te,he,u,q,d,p,te,s,le,E,ie,J,B,m,_,x,H,v,t);f.useEffect(()=>{X&&!i&&(h(X),v(`Updated folder ID from upload processor: ${X}`))},[X,i]),f.useEffect(()=>{N(a=>{const A=new Set;return a.forEach(D=>{D<P.length&&A.add(D)}),A}),G(a=>{const A=new Map(a),D=[];return a.forEach((C,Q)=>{Q>=P.length&&D.push(Q)}),D.forEach(C=>{A.delete(C)}),A})},[P.length]),f.useEffect(()=>{oe(a=>{const A=new Set;return a.forEach(D=>{D<J.length&&A.add(D)}),A}),ge(a=>{const A=new Map(a),D=[];return a.forEach((C,Q)=>{Q>=J.length&&D.push(Q)}),D.forEach(C=>{A.delete(C)}),A})},[J.length]);const ht=a=>{v(`Removing photo at index: ${a}`);const A=P.filter((D,C)=>C!==a);x(A),v(`New files count: ${A.length}`),A.length>0?(localStorage.setItem(me.SELECTED_PHOTOS,JSON.stringify(A)),v(`Updated localStorage with ${A.length} photos`)):(localStorage.removeItem(me.SELECTED_PHOTOS),v("Removed photos from localStorage")),N(D=>{const C=new Set;return D.forEach(Q=>{Q<a?C.add(Q):Q>a&&C.add(Q-1)}),C}),G(D=>{const C=new Map;return D.forEach((Q,fe)=>{fe<a?C.set(fe,Q):fe>a&&C.set(fe-1,Q)}),C})},bt=a=>{v(`Removing existing file at index: ${a}`);const A=J.filter((D,C)=>C!==a);V(A),v(`New existing files count: ${A.length}`),oe(D=>{const C=new Set;return D.forEach(Q=>{Q<a?C.add(Q):Q>a&&C.add(Q-1)}),C}),ge(D=>{const C=new Map;return D.forEach((Q,fe)=>{fe<a?C.set(fe,Q):fe>a&&C.set(fe-1,Q)}),C})},yt=a=>{v(`Toggling selection for existing file at index: ${a}`),oe(A=>{const D=new Set(A);return D.has(a)?(D.delete(a),v(`Deselected existing file ${a} - remaining selected: ${D.size}`)):(D.add(a),v(`Selected existing file ${a} - total selected: ${D.size}`)),D})},wt=()=>{v(`Selecting all ${J.length} existing files`);const a=new Set;for(let A=0;A<J.length;A++)a.add(A);oe(a),v(`Selected all existing files - total: ${a.size}`)},Tt=()=>{v("Deselecting all existing files"),oe(new Set)},$t=a=>{v(`Toggling selection for photo at index: ${a}`),N(A=>{const D=new Set(A);return D.has(a)?(D.delete(a),v(`Deselected photo ${a} - remaining selected: ${D.size}`)):(D.add(a),v(`Selected photo ${a} - total selected: ${D.size}`)),D})},St=()=>{v(`Selecting all ${P.length} photos`);const a=new Set;for(let A=0;A<P.length;A++)a.add(A);N(a),v(`Selected all photos - total: ${a.size}`)},vt=()=>{v("Deselecting all photos"),N(new Set)},It=()=>{confirm(t("Are you sure you want to delete all new files? This action cannot be undone."))&&(v("Deleting all new photos"),x([]),N(new Set),G(new Map),localStorage.removeItem(me.SELECTED_PHOTOS),v("Cleared all photos from localStorage"))};f.useEffect(()=>{v("Selection state changed:",{selectedPhotos:Array.from(T),selectedExistingFiles:Array.from(B),totalSelected:T.size+B.size})},[T,B,v]);const At=()=>{const a=!d;v(`Toggling isOnPublicProfile to: ${a}`),ee(a)},Pt=()=>{const a=!p;v(`Toggling participantsCanAddItems to: ${a}`),Y(a)},jt=()=>{const a=!te;v(`Toggling participantsCanDeleteItems to: ${a}`),O(a)},kt=async()=>{v("Album save initiated"),v("Photo tags applied:",Object.fromEntries(E)),v("Existing file tags applied:",Object.fromEntries(ie)),F(!1),m(!0);try{if(Ne!=null&&Ne.startsWith("Profile-")){v("Public username starts with 'Profile-', showing username prompt"),K(""),g(!0),m(!1);return}v("Valid username found, proceeding to save album directly with file-level tagging"),We()}catch(a){console.error("Error in handleSaveAlbum:",a),v(`Error in handleSaveAlbum: ${a}`),m(!1)}},Dt=a=>{v(`Handling successful username update to: ${a}`),localStorage.setItem(me.PUBLIC_USERNAME,a),mt(a),g(!1),v("Proceeding to save album after username update"),We()},Ct=(a,A)=>{v(`Password dialog closed with option: ${a}, password: ${A?"******":"undefined"}`),a&&ae(a),A!==void 0&&M(A),ce(!1)},Et=()=>{v("Opening password dialog"),ce(!0)},Ft=()=>{v("Add photos button clicked"),pe(i)},_t=()=>{F(!b)},De=a=>{a(),F(!1)};f.useEffect(()=>{const a=A=>{const D=A.target;b&&!D.closest(".settings-dropdown-container")&&F(!1)};return b&&document.addEventListener("mousedown",a),()=>{document.removeEventListener("mousedown",a)}},[b]);const ze=$||l||r,Nt=P.length>0||J.length>0,zt=T.size>0||B.size>0,Rt=we.useMemo(()=>{const a=T.size+B.size;return v(`Total selected files count updated: ${a}`),a},[T.size,B.size,v]),Ot=we.useMemo(()=>{const a=Array.from(E.entries()).map(([D,C])=>`${D}:${C.map(Q=>`${Q.tagTitle}(${Q.subtags.map(fe=>fe.subtagTitle).join(",")})`).join("|")}`).join(";"),A=Array.from(ie.entries()).map(([D,C])=>`${D}:${C.map(Q=>`${Q.tagTitle}(${Q.subtags.map(fe=>fe.subtagTitle).join(",")})`).join("|")}`).join(";");return`${a}||${A}`},[E,ie]);return e.jsxs(e.Fragment,{children:[e.jsx(ns,{}),e.jsx(ls,{children:e.jsxs(ds,{children:[e.jsx(cs,{href:"my-albums.html",children:t("My Albums")}),e.jsxs("div",{style:{display:"flex",alignItems:"center",gap:"12px"},children:[re===!0&&e.jsxs("div",{className:"settings-dropdown-container",style:{position:"relative",display:"flex",alignItems:"center"},children:[e.jsx("button",{onClick:_t,disabled:$||l||r,style:{background:"none",border:"none",cursor:$||l||r?"not-allowed":"pointer",padding:"8px",borderRadius:"4px",display:"flex",alignItems:"center",justifyContent:"center",color:$||l||r?"#ccc":"#666",fontSize:"18px",transition:"all 0.2s ease",backgroundColor:b?"#f0f0f0":"transparent",opacity:$||l||r?.5:1},onMouseEnter:a=>{!b&&!$&&!l&&!r&&(a.currentTarget.style.backgroundColor="#f8f9fa")},onMouseLeave:a=>{!b&&!$&&!l&&!r&&(a.currentTarget.style.backgroundColor="transparent")},title:t("Album Settings"),children:"⚙️"}),b&&!$&&!l&&!r&&e.jsxs(us,{children:[e.jsx(Ee,{onClick:()=>De(At),children:e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"},children:[e.jsx("span",{children:t(d?"Remove From Public Profile":"Add To Public Profile")}),e.jsx("span",{style:{fontSize:"12px",color:d?"#28a745":"#6c757d",fontWeight:"bold"},children:d?"✓":"○"})]})}),e.jsx(Ee,{onClick:()=>De(Pt),children:e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"},children:[e.jsx("span",{children:t(p?"Allow Additions":"Do Not Allow Additions")}),e.jsx("span",{style:{fontSize:"12px",color:p?"#28a745":"#6c757d",fontWeight:"bold"},children:p?"✓":"○"})]})}),e.jsx(Ee,{onClick:()=>De(jt),children:e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"},children:[e.jsx("span",{children:t(te?"Allow Removals":"Do Not Allow Removals")}),e.jsx("span",{style:{fontSize:"12px",color:te?"#28a745":"#6c757d",fontWeight:"bold"},children:te?"✓":"○"})]})}),e.jsx(Ee,{onClick:()=>De(Et),children:e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"},children:[e.jsx("span",{children:t("Album Password Policy")}),e.jsx("span",{style:{fontSize:"12px",color:s!=="NoPassword"?"#28a745":"#6c757d",fontWeight:"bold"},children:s!=="NoPassword"?"✓":"○"})]})})]})]}),e.jsx(Ze,{$primary:!0,onClick:kt,disabled:$||l||r,style:{minWidth:"120px",fontSize:"14px",padding:"8px 16px"},children:t($?"Saving...":"Save Album")})]})]})}),e.jsxs(ps,{$isRTL:n,children:[e.jsx("div",{style:{marginTop:L&&re===!0?"3px":"0"},children:e.jsx(er,{showFolderDetails:L,isCreator:re,folderName:u,setFolderName:o,folderDescription:q,setFolderDescription:R,isSavingAlbum:$||l})}),(l||z.totalFiles>0&&(z.filesUploading>0||z.filesProcessing>0||z.filesComplete<z.totalFiles))&&e.jsx(hs,{progressTracker:z,isRTL:Me(c)==="rtl",variant:"detailed",context:"saving",isUploading:l,showSuccessMessage:!1,showErrorMessage:!0,customMessages:{error:t("Some photos could not be processed. You can continue with the successfully processed photos.")}}),r&&e.jsx("div",{style:{padding:"16px",marginBottom:"16px",backgroundColor:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:"8px",color:"#6c757d",textAlign:"center",fontStyle:"italic"},children:t("Loading existing files...")}),e.jsx("input",{ref:W,id:"file-input",type:"file",accept:"image/*,video/*",multiple:!0,onChange:a=>$e(a,Ue),style:{display:"none"}}),L&&re===!0&&Nt&&e.jsxs(nr,{t,isRTL:n,children:[J.length>0&&e.jsx("div",{style:{marginBottom:"32px"},children:e.jsx(dr,{existingFiles:J,selectedExistingIndices:B,onToggleSelection:yt,onSelectAll:wt,onDeselectAll:Tt,onDeleteFile:bt,disabled:ze,isCreator:re,participantsCanDeleteItems:te,existingFileTagsMap:ie,t,isRTL:n})}),e.jsx(lr,{selectedPhotos:P,selectedPhotoIndices:T,onToggleSelection:$t,onSelectAll:St,onDeselectAll:vt,onRemovePhoto:ht,onDeleteAll:It,disabled:ze,photoTagsMap:E,t,isRTL:n}),e.jsx("div",{style:{marginTop:zt?"32px":"16px"},children:e.jsx(Ys,{tagsManager:xt,disabled:ze,enhancedLog:v},`tags-${Rt}-${Ot.slice(0,20)}`)})]}),e.jsx(Ls,{isSavingAlbum:$,savingProgress:k}),e.jsx(fs,{children:e.jsx(Ze,{onClick:Ft,disabled:$||l||r,children:t(l?"Uploading...":"Add More Photos")})}),e.jsx(ms,{t,language:c,usernameManager:w,onSuccess:Dt}),e.jsx(Os,{isOpen:se,onClose:Ct,initialOption:s,initialPassword:le}),e.jsx(Ms,{debugMessages:j,t,isRTL:n,textDirection:n?"rtl":"ltr"})]})]})},ur=()=>e.jsx(Qt,{children:e.jsx(cr,{})});Jt.createRoot(document.getElementById("root")).render(e.jsx(ur,{}));
